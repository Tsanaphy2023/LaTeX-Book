...
% ==========================================================
% Chapter 5 Instructional Management Plan & Syllabus
% ==========================================================
\section*{Chapter 5 Instructional Management Plan}
\addcontentsline{toc}{section}{Chapter 5 Instructional Management Plan}
...
\begin{enumerate}[topsep=2pt, itemsep=1pt, partopsep=0pt, parsep=0pt, leftmargin=1.5em]
    \item Matrix Operations and Special Classes of Matrices
    \item The Eigenvalue Problem and Diagonalization
    \item Physical Applications: Coupled Oscillators and Normal Modes
    \item Worked Examples and Inertia Tensor Transformations
...
\begin{enumerate}[topsep=2pt, itemsep=1pt, partopsep=0pt, parsep=0pt, leftmargin=1.5em]
    \item Perform matrix algebra, determinant evaluations, and matrix inversions.
    \item Solve the secular eigenvalue equation $\det(\mathbf{A} - \lambda\mathbf{I}) = 0$ for eigenvalues and eigenvectors.
    \item Diagonalize symmetric and Hermitian operators via similarity transformations.
    \item Model coupled mechanical oscillators, normal modes, and inertia tensors.
\end{enumerate}
\end{planbox}
...
\begin{enumerate}[topsep=2pt, itemsep=1pt, partopsep=0pt, parsep=0pt, leftmargin=1.5em]
    \item Lecture on linear vector spaces, matrix operators, and spectral decomposition.
    \item Workshop deriving normal mode frequencies and eigenvectors for multi-mass systems.
    \item Computational lab using Python `numpy.linalg` for large-scale matrix diagonalization.
    \item Case study on principal axes of inertia in rigid body rotational dynamics.
\end{enumerate}
\end{planbox}

\begin{planbox}[colframe=rbruGold!90!black, colbacktitle=rbruGold!90!black]{4. Instructional Media and Educational Technology}
\begin{enumerate}[topsep=2pt, itemsep=1pt, partopsep=0pt, parsep=0pt, leftmargin=1.5em]
    \item Course textbook: Chapter 5 Linear Algebra \& Matrix Mechanics.
    \item Lecture slides illustrating 2D/3D geometric transformations and eigenvector shears.
    \item Python scripts for matrix operations and coupled oscillator animations.
    \item Digital problem sets and matrix mechanics worksheets.
\end{enumerate}
\end{planbox}

\begin{planbox}[colframe=rbruSlate, colbacktitle=rbruSlate]{5. Measurement and Learning Assessment}
\begin{enumerate}[topsep=2pt, itemsep=1pt, partopsep=0pt, parsep=0pt, leftmargin=1.5em]
    \item In-class engagement and formative assessment of matrix transformation proofs.
    \item Evaluation of homework problem sets on eigenvalues and coupled oscillators.
    \item Chapter 5 diagnostic examination on matrix diagonalization and normal modes.
\end{enumerate}
\end{planbox}
\clearpage
...
\begin{psctexample}[Diagonalization of a 2D Moment of Inertia Tensor]
...
For mass 1 at $(a, 0, 0)$: $|\vec{r}_1|^2 = a^2$.
For mass 2 at $(0, b, 0)$: $|\vec{r}_2|^2 = b^2$.

Assembling the relevant $2\times 2$ block in the $xy$-plane:
\begin{align}
I_{xx} &= m(0^2) + m(b^2) = mb^2 \\
I_{yy} &= m(a^2) + m(0^2) = ma^2 \\
I_{xy} &= I_{yx} = -m(a)(0) - m(0)(b) = 0
\end{align}
The inertia tensor (in the $xy$-plane) is diagonal:
\begin{equation}
\mathbf{I} = m\begin{pmatrix} b^2 & 0 \\ 0 & a^2 \end{pmatrix}
\end{equation}
...
\begin{equation}
\det\begin{pmatrix} mb^2 - \lambda & -mab/2 \\ -mab/2 & ma^2 - \lambda \end{pmatrix} = 0
\implies \lambda^2 - m(a^2 + b^2)\lambda + m^2\!\left(a^2b^2 - \frac{a^2b^2}{4}\right) = 0
\end{equation}
\begin{equation}
\boxed{\lambda_{1,2} = \frac{m(a^2 + b^2)}{2} \pm \frac{m}{2}\sqrt{(a^2 - b^2)^2 + a^2b^2}}
\end{equation}

\textbf{\color{rbruNavy}Think / Physical Insights:}
Diagonalizing $\mathbf{I}$ is physically equivalent to finding a body frame in which the angular momentum $\vec{L} = \mathbf{I}\vec{\omega}$ is always parallel to $\vec{\omega}$. This frame defines the principal axes. For a free rigid body, rotation about the largest and smallest principal moments is stable (Tennis Racket Theorem).
\qedexam
\end{psctexample}
...
\begin{enumerate}
    ...
    \item \textbf{Cayley-Hamilton Theorem:} Every square matrix satisfies its own characteristic equation. Verify this for $\mathbf{A} = \begin{pmatrix} 3 & 1 \\ 2 & 4 \end{pmatrix}$ by computing $\mathbf{A}^2 - 7\mathbf{A} + 10\mathbf{I}$ and confirming it equals the zero matrix.
    \item \textbf{Singular Value Decomposition (SVD):} Explain what the singular values $\sigma_i$ of a real matrix $\mathbf{M}$ represent geometrically. For the matrix $\mathbf{M} = \begin{pmatrix} 3 & 0 \\ 4 & 5 \end{pmatrix}$, compute $\mathbf{M}^T\mathbf{M}$ and find its eigenvalues, which are $\sigma_i^2$.
\end{enumerate}
